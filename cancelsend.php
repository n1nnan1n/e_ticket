...
if (!isset($_SESSION['id'])) {
  ...
  foreach($result as $u)
  {
    if (!isset($_GET['tid']))
    {
      ...
      $tid = $_GET['tid'];
      $check = getOneUserSendingTicket($_SESSION['id'],$tid);
          ...
          if ($_SESSION['id'] != $c['tk_user_id']){
			...
			if(isset($_POST['submit'])){
				...
				if (password_verify($password, $u['user_password'])) {
                    cancelsend($con,$tid,$_SESSION['id'],$event,$point);
					  header("Location: myticket.php");
					}else{
			...
			<div class="card-content-detail">
				<div class="card-body" style="box-shadow: 1px 1px 1px #adadad; border-radius: 15px;">
					...
					<div class="text-left">
                        ...
                        - ผู้รับบัตรจะไม่สามารถรับบัตรได้<br>
					</div>
				</div>
			</div>
		</div>
		<div class="row mt-2">
							...
							<form action="" method="POST">
                                    <div class="form-group">
                                      <button type="button" class="btn btn-buy" data-bs-toggle="modal" data-bs-target="#exampleModal">ยกเลิกการส่ง</button>
