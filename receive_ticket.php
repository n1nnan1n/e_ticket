...
if (!isset($_SESSION['id'])) {
  ...
  foreach($result as $u)
  {
    if (!isset($_GET['tid']))
    {
      ...
      $tid = $_GET['tid'];
      $check = getOneUserReceiveticket($_SESSION['id'],$tid);
        ...
        $point = $c['ticket_price'];
          ...
          if ($_SESSION['id'] != $c['tk_customer_id']){
            ...
            if(isset($_POST['receive'])){
                receive_ticket($con,$tid,$sender,$receiver,$point);
                header("Location: myticket.php");
            }elseif(isset($_POST['reject'])){
                reject_ticket($con,$tid,$sender,$receiver,$point);
                header("Location: myticket.php");
            }
        }
    }
                        ...
                        <div class="w3-card " id="fill">
                            <div class="w3-container w3-center">
                                <form method="POST" action="">
                                    <button type="submit" name="receive" class="btn btn-buy" style="background-color:green;">รับบัตร</button>
                                    <button type="submit" name="reject" class="btn btn-buy">ไม่รับบัตร</button>
                                </form>
                            </div>
                        </div>
                    </div>
